// src/components/sections/AboutValuesSection.tsx
"use client";
...
const OUR_IMPACT = [
  ...
  { icon: "fa-cloud", label: "Reducing methane emissions at source" },
  {
    ...
    label: "Creating circular economic value",
  },
...
function SolutionCard({
  ...
  return (
    <div
      className={`rounded-lg p-8 flex flex-col gap-5 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
      style={{
        background: sol.bg,
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          <i
            className={`fas ${sol.icon} text-[0.95rem]`}
            style={{ color: sol.accent }}
          />
        </div>
        {sol.badge && (
          <span
            className="font-mono text-[0.6rem] tracking-[0.12em] uppercase px-2.5 py-1 rounded-pill"
            style={{
      ...
      <div>
        <h4
          className="font-display font-semibold text-[1.3rem] mb-3"
...
export default function AboutValuesSection() {
  ...
  return (
    <section
      className="relative py-[var(--section-py-lg)] px-12 overflow-hidden"
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 40% at 50% 100%, var(--about-values-glow) 0%, transparent 70%)",
        }}
      />
      ...
      <div ref={ref} className="relative z-10 max-w-[1280px] mx-auto">
        ...
        <div
          className={`grid grid-cols-1 md:grid-cols-2 gap-7 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        ...
        >
          ...
          >
            ...
            <p
              className="text-[0.88rem] leading-[1.8] mb-8"
              style={{ color: "var(--text-secondary)" }}
            >
              We collaborate with organizations committed to reducing their
            ...
            <ul className="flex flex-col gap-4">
              {WHO_WE_SERVE.map(({ icon, label }) => (
                <li
                  key={label}
                  className="flex items-center gap-4 text-[0.9rem]"
                  style={{ color: "var(--text-secondary)" }}
                >
                  <span
                  ...
                  >
                    <i
                      className={`fas ${icon} text-[0.7rem]`}
                      style={{ color: "var(--coffee-latte)" }}
                    />
                  </span>
                  {label}
          ...
          >
            ...
            <p
              ...
              style={{ color: "var(--text-secondary)" }}
            >
              Every kilogram of waste we collect triggers a measurable chain of
              positive outcomes.
            </p>
            <div
              className="h-px mb-8"
              style={{ background: "var(--about-step-divider)" }}
            />
            <ul className="flex flex-col gap-4">
